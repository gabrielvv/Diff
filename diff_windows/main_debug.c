    ...
    while(myCustomFile2->lines_count-start > result_list2->length){
      ...
      for(i = start; i < length2; i++){
        ...
        for(j = offset; j < length1; j++){
          //printf("Compare %s et %s\n",str,myCustomFile->lines_content[j]);
          if(strcmp(str,myCustomFile->lines_content[j]) == 0){
            //printf("match\n");
            if(store){
              int index = ((int)(tmp_list2->length-1) < 0 ? 0 : (int)(tmp_list2->length-1));
              //printf("Replace %d by %d\n", tmp_list2->datas[index], i+1 );
              //printf("Index: %d\n", index);
              tmp_list1->datas[index] = j+1;//ss sequ file1
              tmp_list2->datas[index] = i+1; //ss sequ file2
              ...
              //list_display(tmp_list2);
              list_append(tmp_list1, j+1); //incrémente list->length
